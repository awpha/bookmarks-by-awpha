...
function awpha_bookmarks_favicon( $url, $save=false, $path='./' ) {

$path 	= (isset($path)) ? $path : awpha_bookmarks_get_upload_dir_var('basedir', 'br_favicons');

	$grap_favicon = array(
		'URL' => $url,   // URL of the Page we like to get the Favicon from
		'SAVE'=> $save,  // Save Favicon copy local (true) or return only favicon url (false)
		'DIR' => $path,  // Local Dir the copy of the Favicon should be saved
		'TRY' => true,   // Try to get the Favicon from the page (true) or only use the APIs (false)
		'DEV' => null,   // Give all Debug-Messages ('debug') or only make the work (null)
	);

	$favicon = awpha_bookmarks_grap_favicon($grap_favicon);
...
function awpha_bookmarks_grap_favicon( $options=array() ) {
  ...
  if ( !file_exists($filePath) or @filesize($filePath)==0 ) {
	...
	if ( isset($trySelf) and $trySelf == TRUE ) {  
	  ...
	  if ( @preg_match($regExPattern, $html, $matchTag) ) {
		...
		if ( isset($matchTag[1]) and @preg_match($regExPattern, $matchTag[1], $matchUrl)) {
		  if ( isset($matchUrl[2]) ) {
			...
			// Build Favicon Link
			$favicon = awpha_bookmarks_rel2abs(trim($matchUrl[2]), 'https://'.$domain.'/'); //http://
	  ...
	  // If there is no Match: Try if there is a Favicon in the Root of the Domain
		if ( empty($favicon) ) { 
	  	$favicon = 'https://'.$domain.'/favicon.ico'; //http://
...
function awpha_bookmarks_rel2abs( $rel, $base ) {
